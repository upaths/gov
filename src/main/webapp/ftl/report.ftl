...
<div class="content">
    ...
    <div class="content-tijiao">
        <div class="forms">
            <form action="${contextPath}/report_insert/" id="report_form" method="post" enctype="multipart/form-data">
                <p class="jbfs">
                    <input  name="jubao" type="radio"  value="0" checked />实名举报
                    <input type="radio" name="jubao"  value="1"   />匿名举报</p>
                <div class="form-show">
                    <p class="f-bt">反映人信息</p>
                    <p><label for="">姓名：</label><input type="text" name="report.reportName" maxlength="50" /></p>
                    <p><label for="">联系电话：</label><input type="text" name="report.reportContact" maxlength="50" /></p>
                    <p><label for="">工作单位（住址）及职务：</label><input type="text" name="report.reportCondition" maxlength="100"  style="width:460px;" /></p>
                    <div class="line"></div>
                </div>
                <p class="f-bt">被反映人信息</p>
                <p><label for="">姓名：</label><input type="text" name="report.reportedName" maxlength="50" /></p>
                <p><label for="">工作单位（住址）及职务：</label><input type="text" name="report.reportedCondition" maxlength="100"  style="width:460px;" /></p>
                <div class="line"></div>
                <p class="f-bt">反映问题内容</p>
                <p><label for="">标题（最多50字）</label><textarea name="report.title" rows="1" cols="30" maxlength="50"></textarea></p>
                <p><label for="">主要问题（1000字以内）：</label><textarea name="report.problem" rows="5" cols="30" maxlength="1000"></textarea></p>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $("#report_form").validate({
            rules: {
                "report.reportedName": {required: true, maxlength:50},
                "report.reportedCondition": {required: true, maxlength:100},
                "report.title": {required: true, maxlength:50},
                "report.problem": {required: true, maxlength:1000},
                "rand": {required: true}
            }
        });
    });
