 ...
 */
public class TopicCommentListDirective implements TemplateDirectiveModel {
    private TopicService topicService;
    private static final String TOPICID_NAME = "topid";
    ...
    public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
        ...
        while (paramIterator.hasNext()) {
            ...
            TemplateModel paramValue = (TemplateModel)
                    ent.getValue();
            if (paramName.equals(TOPICID_NAME)) {
                topid = DataChecker.checkInt(paramValue, paramName);
        ...
        if (topid < 0) {
            throw new TemplateModelException("参数" + TOPICID_NAME + "不能为空");
        }
        if (page > 0 && size < 0) {
            throw new TemplateModelException("参数" + PAGE_NUMBER_NAME + "必须与" + PAGE_SIZE_NAME + "同时存在");
        }
        List<TopicComment> topicCommentList = topicService.queryDisplayTopicComment(topid, sort, page, size);
        int cnt = topicService.countDisplayTopicComment(topid);
        // 执行真正指令的执行部分:
        if (body != null) {
            // 如果有循环变量，那么就设置它:
            if (loopVars.length > 0) {
                // 数据
                loopVars[0] = ObjectWrapper.DEFAULT_WRAPPER.wrap(topicCommentList);
